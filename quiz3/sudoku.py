...
def get_data(i, j, lines):
  ...
  if j in [0, 3, 6]:
    ...
    j = j - 1
  arroud_list = [lines[i][j], lines[i - 1][j], lines[i + 1][j], lines[i][j - 1], lines[i - 1][j - 1], lines[i + 1][j - 1], lines[i][j + 1], lines[i - 1][j + 1], lines[i + 1][j + 1]]
...
if __name__ == "__main__":
  ...
  with open(sys.argv[4], 'w', newline='') as f:
    ...
    for i in range(9):
      for j in range(9):
        if lines_[i][j] == 0:
          L = (i, j, get_data(i, j, lines_))
          data.append(L)
    File.writerow(['row', 'column', 'numbers'])
    File.writerows(data)
